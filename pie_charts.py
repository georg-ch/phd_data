...
from plotting_style import short_faculty_label
from plotting_traces import sorted_with_rest
...
def order_pie_categories(d):
    ...
    order = {name: i for i, name in enumerate(sorted_with_rest(d["category_name"].unique()))}
    return d.assign(_order=d["category_name"].map(order)).sort_values(
...
def write_category_pie_interactive(
    ...
    order_categories = order_pie_categories_by_size if order_by_size else order_pie_categories
    ...
    for faculty in faculty_list:
        ...
        fig.add_trace(
            go.Pie(
                ...
                sort=False,
                marker=dict(
                    colors=[category_colors.get(x, restcolor) for x in dff["category_name"]],
                    line=dict(color="rgba(0,0,0,0)", width=0),
                ),
