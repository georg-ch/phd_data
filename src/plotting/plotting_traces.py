...
from src.plotting.plotting_style import faculty_color_for, short_faculty_label


def reindex_years(
    df: pd.DataFrame, years_sorted, year_col: str = "year"
) -> pd.DataFrame:
...
def add_count_text(
    ...
    *,
    ...
    out = df.copy()
    out[out_col] = [
        f"{int(value):,} {singular_name}"
        if value == 1
        else f"{int(value):,} {plural_name}"
